...
const int = FormValue.of(123, { validate: (x) => x > 0 })
const str = FormValue.of("alsd")
const obj = FormValue.of({
  first: 1,
  second: [""],
})

const list = FormList.of([int])

const shp = FormShape.of(
  {
    jkl: FormValue.of(123),
    bar: int,
    foo: str,
    baz: obj,
    toz: list,
  ...
  {
    validate: (x) => x.bar > 0 && x.foo.length < 10 && x.baz.first < 100,
  },
)
...
shp.setFoo((x) => {
  x.bar.setValue((bar) => bar + 1)
  x.toz.setFoo((toz) => {
    toz.at(0)?.setValue((l) => l + 1)
  })
})
